            ...
            \item Hypothèses de la méthode : 
                \begin{itemize}
                    \item 
                        ...
                        \end{itemize}
                    \item
                        La fonction $f(x)$ doit être continue dans l'intervalle $[x_0 , x_1]$, soit $f \in C_0([x_0,x1])$. Malheureusement, aucune fonction des bibliothèque autorisées ne permet de vérifier ce caractère. Supposons pour le projet que la fonction $f(x)$ dont on cherche la racine est bien continue sur l'intervalle. Nous verrons par la suite que cette hypothèse est acceptable car nous étudions un phénomène physique d'évolution de température dans un espace, en trois dimensions, continu, de sorte que la fonction est bien continue sur l'intervalle et l'hypothèse est vérifiée.
...
\section{Régulation automatisée du bâtiment}
    ...
    \subsection{$\Delta t_{optimal}$ pour 24 heures}\label{section:Delta_t_optimal}
        ...
        La détermination de ce $\Delta t_{optimal}$ est représenté à la figure \ref{fig:T_confort_optimal}.
        \begin{figure}
            \centering
